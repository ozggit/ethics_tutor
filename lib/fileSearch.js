...
export async function uploadToFileSearchStore({ buffer, filename, mimeType }) {
  ...
  const uploadUrl =
    ...
    `https://generativelanguage.googleapis.com/v1beta/fileSearchStores/${FILE_SEARCH_STORE_NAME}/files:upload?key=${GEMINI_API_KEY}`;
  ...
  if (!response.ok) {
    ...
    throw new Error(`File Search upload failed: ${text}`);
  }

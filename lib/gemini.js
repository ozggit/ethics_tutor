...
const OUTPUT_PROMPT =
  "ענה/י בעברית טבעית, חמה ומעודדת. כתוב/י בפסקאות קצרות, עם Markdown עדין כשזה עוזר. " +
  "אפשר להיות מעט יותר מפורט/ת (בערך 6-12 משפטים), אבל אם החומר המצורף קצר/חלקי, " +
  "תן/י תשובה קצרה ומדויקת לפי מה שנמצא בלבד. " +
  ...
  "אל תכלול/י סימוני ציטוט/עוגנים בתוך הטקסט כמו [cite: 1, 2, 3]. " +
  "אל תחזור/י על אותה תשובה פעמיים; אם כבר הסברת נקודה, אל תשכפל/י פסקאות.";
...
export async function generateAnswer({ userText, systemInstruction, fileSearchConfig }) {
  ...
  async function requestGenerateContent(variant, opts = {}) {
    ...
    const maxOutputTokens = opts.maxOutputTokens ?? 1100;
  ...
  async function requestWithModelFallback(variant, opts = {}) {
    ...
    try {
      return await requestWithThinkingFallback(variant, {
        ...
        maxOutputTokens: Math.max(opts.maxOutputTokens ?? 1100, 1400)
      });
    } catch {
      return primary;
    }
